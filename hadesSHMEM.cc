...
HadesSHMEM::HadesSHMEM(Component *owner, Params &params) :
    ...
    if (famMapperParams.size()) {
        m_famNodeMapper = dynamic_cast<FamNodeMapper *>( loadModule(
            famMapperParams.find<std::string>("name"), famMapperParams));
        m_famNodeMapper->setDbg(&m_dbg);
    }

    famMapperParams = params.find_prefix_params("famAddrMapper.");
    if (famMapperParams.size()) {
        m_famAddrMapper = dynamic_cast<FamAddrMapper *>( loadModule(
            famMapperParams.find<std::string>("name"), famMapperParams));
...
void HadesSHMEM::memcpy(Hermes::Vaddr dest, Hermes::Vaddr src, size_t length,
                        Shmem::Callback callback) {
    dbg().debug(CALL_INFO, 1, SHMEM_BASE, "dest=%#"
    PRIx64
    " src=%#"
    PRIx64
    " length=%zu\n", dest, src, length);
...
void HadesSHMEM::init(Init *info) {
    ...
    malloc(&m_localData, m_localDataSize, true,
           [=]() {
               ...
               dbg().debug(CALL_INFO, 1, SHMEM_BASE, "pSync=%"
               PRIx64
               " PutCnt=%"
               PRIx64
               " , GetCnt=%"
...
void HadesSHMEM::malloc(Hermes::MemAddr *ptr, size_t size, bool backed, Callback callback) {
    ...
    if (m_memHeapLink) {
        m_memHeapLink->alloc(size,
                             [=](uint64_t addr) {
                                 this->dbg().debug(CALL_INFO_LAMBDA, "malloc", 1, SHMEM_BASE,
                                                   "addr=%#"
                                 PRIx64
                                 " size=%zu\n", addr, size);
...
void HadesSHMEM::broadcast(Vaddr dest, Vaddr source, size_t nelems, int root, int start,
    ...
    Broadcast *info = new Broadcast(dest, source, nelems, root, start, stride, size, pSync,
                                    callback);
    delayEnter(
        [=]() {
            this->broadcast(info);
...
void HadesSHMEM::broadcast(Broadcast *info) {
    dbg().debug(CALL_INFO, 1, SHMEM_BASE, "\n");
    m_broadcast->start(info->dest, info->source, info->nelems, info->root, info->PE_start,
...
void HadesSHMEM::fcollect(Fcollect *info) {
    dbg().debug(CALL_INFO, 1, SHMEM_BASE, "\n");
    m_fcollect->start(info->dest, info->source, info->nelems, info->PE_start, info->logPE_stride,
                      info->PE_size, info->pSync,
                      [=](int) {
                          this->delayReturn(info->callback);
                          delete info;
                      }
...
void HadesSHMEM::collect(Collect *info) {
    dbg().debug(CALL_INFO, 1, SHMEM_BASE, "\n");
    m_collect->start(info->dest, info->source, info->nelems, info->PE_start, info->logPE_stride,
                     info->PE_size, info->pSync, &m_localScratch,
                     [=](int) {
                         this->delayReturn(info->callback);
                         delete info;
                     }
...
void HadesSHMEM::alltoall(Alltoall *info) {
    dbg().debug(CALL_INFO, 1, SHMEM_BASE, "\n");
    m_alltoall->start(info->dest, info->source, info->nelems, info->PE_start, info->logPE_stride,
                      info->PE_size, info->pSync,
                      [=](int) {
                          this->delayReturn(info->callback);
                          delete info;
...
void HadesSHMEM::alltoalls(Vaddr dest, Vaddr source, int dst, int sst, size_t nelems, int elsize,
                           int start,
                           int stride, int size, Vaddr pSync, Shmem::Callback callback) {
    Alltoalls *info = new Alltoalls(dest, source, dst, sst, nelems, elsize, start, stride, size,
                                    pSync, callback);
    delayEnter(
        [=]() {
            this->alltoalls(info);
...
void HadesSHMEM::alltoalls(Alltoalls *info) {
    dbg().debug(CALL_INFO, 1, SHMEM_BASE, "\n");
    m_alltoalls->start(info->dest, info->source, info->dst, info->sst, info->nelems, info->elsize,
                       info->PE_start,
                       info->logPE_stride, info->PE_size, info->pSync,
                       [=](int) {
                           this->delayReturn(info->callback);
...
void HadesSHMEM::reduction(Vaddr dest, Vaddr source, int nelems, int PE_start,
                           ...
                           Shmem::Callback callback) {
    Reduction *info = new Reduction(dest, source, nelems, PE_start, logPE_stride, PE_size, pSync,
...
void HadesSHMEM::reduction(Reduction *info) {
    dbg().debug(CALL_INFO, 1, SHMEM_BASE, "\n");
    m_reduction->start(info->dest, info->source, info->nelems, info->PE_start, info->logPE_stride,
                       info->PE_size, info->pSync,
                       info->op, info->type,
                       [=](int) {
                           this->delayReturn(info->callback);
...
void HadesSHMEM::get(Hermes::Vaddr dest, Hermes::Vaddr src, size_t length, int pe, bool blocking,
                     Shmem::Callback callback) {
    dbg().debug(CALL_INFO, 1, SHMEM_BASE, "destSimVAddr=%#"
    PRIx64
    " srcSimVaddr=%#"
    PRIx64
    " length=%lu\n",
...
void HadesSHMEM::getv(Getv *info) {
    dbg().debug(CALL_INFO, 1, SHMEM_BASE, "srcSimVaddr=%#"
    PRIx64
    "\n", info->src );
    ...
    nic().shmemGetv(calcNetPE(info->pe), info->src, info->result.getType(),
                    ...
                    [=](Hermes::Value &newValue) {
                        ...
                        ::memcpy(info->result.getPtr(), newValue.getPtr(),
                                 info->result.getLength());
                        this->delayReturn(info->callback, m_blockingReturnLat_ns);
                        delete info;
                    }
    );
...
void HadesSHMEM::put(Hermes::Vaddr dest, Hermes::Vaddr src, size_t length, int pe, bool blocking,
                     Shmem::Callback callback) {
    if (length == 0) {
        delay(callback, 0, 0);
        return;
...
void HadesSHMEM::put(Hermes::Vaddr dest, Hermes::Vaddr src, size_t length, int pe, bool blocking,
                     Shmem::Callback callback, Shmem::Callback &fini) {
...
void HadesSHMEM::putOp(PutOp *info) {
    ...
    nic().shmemPutOp(calcNetPE(info->pe), info->dest, info->src, info->nelems, info->op, info->type,
                     [=]() {
                         this->dbg().debug(CALL_INFO_LAMBDA, "putOp2", 1, SHMEM_BASE,
                                           "returning\n");
                         this->delayReturn(info->callback);
                         delete info;
                     }
...
void HadesSHMEM::wait_until(Hermes::Vaddr addr, Hermes::Shmem::WaitOp op, Hermes::Value &value,
                            Shmem::Callback callback) {
...
void HadesSHMEM::wait_until(WaitUntil *info) {
    std::stringstream tmp;
    tmp << info->value;
    dbg().debug(CALL_INFO, 1, SHMEM_BASE, "addr=%#"
    PRIx64
    " val=%s\n", info->addr, tmp.str().c_str());

    nic().shmemWait(info->addr, info->op, info->value,
                    [=]() {
                        this->dbg().debug(CALL_INFO_LAMBDA, "wait_until2", 1, SHMEM_BASE, "addr=%#"
                        PRIx64
                        " returning\n", info->addr);
                        this->delayReturn(info->callback);
                        delete info;
...
void HadesSHMEM::swap(Hermes::Value &result, Hermes::Vaddr addr, Hermes::Value &value, int pe,
                      Shmem::Callback callback) {
...
void HadesSHMEM::cswap(Hermes::Value &result, Hermes::Vaddr addr, Hermes::Value &cond,
                       Hermes::Value &value, int pe, Shmem::Callback callback) {
...
void HadesSHMEM::cswap(Cswap *info) {
    ...
    nic().shmemCswap(calcNetPE(info->pe), info->addr, info->cond, info->value,
                     ...
                     [=](Hermes::Value &newValue) {
                         this->dbg().debug(CALL_INFO_LAMBDA, "cswap2", 1, SHMEM_BASE,
                                           "returning\n");

                         ::memcpy(info->result.getPtr(), newValue.getPtr(),
                                  info->value.getLength());
...
void HadesSHMEM::fadd(Hermes::Value &result, Hermes::Vaddr addr, Hermes::Value &value, int pe,
                      Shmem::Callback callback) {
...
void HadesSHMEM::fam_add(Shmem::Fam_Descriptor fd, uint64_t offset, Hermes::Value &value,
                         Shmem::Callback &callback) {
...
void HadesSHMEM::fam_cswap(Value &result, Shmem::Fam_Descriptor fd, uint64_t offset,
...
void HadesSHMEM::fam_get(Hermes::Vaddr dest, Shmem::Fam_Descriptor fd, uint64_t offset,
                         uint64_t nbytes,
...
void HadesSHMEM::fam_get2(Hermes::Vaddr dest, Shmem::Fam_Descriptor fd, uint64_t offset,
                          uint64_t nbytes,
                          Shmem::Callback callback, Shmem::Callback finiCallback) {
    FamWork *work = new FamWork(dest, callback, finiCallback);

    m_dbg.debug(CALL_INFO, 1, SHMEM_BASE, "dest=%#"
    PRIx64
    " globalOffset=%#"
    PRIx64
    " nbytes=%"
...
void HadesSHMEM::fam_put(Shmem::Fam_Descriptor fd, uint64_t offset, Hermes::Vaddr src,
                         uint64_t nbytes,
                         bool blocking, Shmem::Callback &callback) {
    if (blocking) {
...
void HadesSHMEM::fam_put2(Shmem::Fam_Descriptor fd, uint64_t offset, Hermes::Vaddr src,
                          uint64_t nbytes,
                          Shmem::Callback callback, Shmem::Callback finiCallback) {
    FamWork *work = new FamWork(src, callback, finiCallback);

    m_dbg.debug(CALL_INFO, 1, SHMEM_BASE, "src=%#"
    PRIx64
    " globalOffset=%#"
    PRIx64
    " nbytes=%"
...
void HadesSHMEM::fam_scatterv(Hermes::Vaddr src, Shmem::Fam_Descriptor fd, uint64_t nElements,
                              std::vector <uint64_t> elementIndexes, uint64_t elementSize,
                              bool blocking, Shmem::Callback &callback) {
    m_dbg.debug(CALL_INFO, 1, SHMEM_BASE, "src=%#"
    PRIx64
    "nElements=%"
...
void HadesSHMEM::fam_scatter(FamVectorWork *work, Hermes::Vaddr src, Shmem::Fam_Descriptor fd,
                             uint64_t nElements,
...
void HadesSHMEM::fam_gather(FamVectorWork *work, Hermes::Vaddr dest, Shmem::Fam_Descriptor fd,
                            uint64_t nElements,
                            uint64_t elementSize, bool blocking, Shmem::Callback &callback) {
    work->addr = dest;
    work->fd = fd;
...
void HadesSHMEM::doFamVectorGet(FamVectorWork *work) {
    ...
    PRIu64
    "\n", work->addr, offset, work->elementSize );
    fam_get2(work->addr + index * work->elementSize, work->fd, offset, work->elementSize, callback,
             finiCallback);
}
