...
public:
    ...
    ShmemRecvEntry(Shmem *shmem, int core, Hermes::MemAddr addr, size_t length) :
        RecvEntryBase() {
        m_shmemMove = new ShmemRecvMoveMem(addr.getBacking(), length, shmem, core,
                                           addr.getSimVAddr());
    }
    ...
    ShmemRecvEntry(Shmem *shmem, int core, Hermes::MemAddr addr, size_t length,
        ...
        RecvEntryBase() {
        m_shmemMove = new ShmemRecvMoveMemOp(addr.getBacking(), length, shmem, core,
                                             addr.getSimVAddr(), op, dataType);
    }
...
public:
    ...
    ShmemGetbRespRecvEntry(Shmem *shmem, int core, size_t length, ShmemGetbSendEntry *entry,
                           void *backing) :
        ShmemGetRespRecvEntry(entry) {
