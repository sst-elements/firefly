...
void FunctionSM::initFunction(SST::Component *obj, Info *info,
                              FunctionEnum num, std::string name, Params &defaultParams,
                              Params &params) {
...
void FunctionSM::processRetval(Retval &retval) {
    if (retval.isExit()) {
        m_dbg.debug(CALL_INFO, 3, 0, "Exit %"
        PRIu64
        "\n", retval.value());
        if (m_retFunc) {
        ...
        m_dbg.debug(CALL_INFO, 3, 0, "Delay %"
        PRIu64
        "\n", retval.value());
