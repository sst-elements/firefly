...
void Nic::SendMachine::streamInit(SendEntryBase *entry) {
    ...
    m_dbg.debug(CALL_INFO, 1, NIC_DBG_SEND_MACHINE,
                ...
                entry->local_vNic(), entry->streamNum(), entry->dest(), entry->dst_vNic(),
                entry->totalBytes());
    ...
    m_nic.schedCallback(std::bind(&Nic::SendMachine::getPayload, this, entry, ev),
                        entry->txDelay());
}
    ...
    if (!m_inQ->isFull()) {
        ...
        if (entry->isDone()) {
            ...
            m_inQ->enque(m_unit, pid, vec, ev, entry->dest(),
                         std::bind(&Nic::SendMachine::streamFini, this, entry));
        } else {
            m_inQ->enque(m_unit, pid, vec, ev, entry->dest());
            m_nic.schedCallback(std::bind(&Nic::SendMachine::getPayload, this, entry,
                                          new FireflyNetworkEvent(m_pktOverhead)), 0);
        }
...
void Nic::SendMachine::streamFini(SendEntryBase *entry) {
    m_dbg.debug(CALL_INFO, 1, NIC_DBG_SEND_MACHINE, "%p sendMachine=%d pid=%d bytes=%zu latency=%"
    PRIu64
...
void Nic::SendMachine::InQ::enque(int unit, int pid, std::vector <MemOp> *vec,
    ...
    m_dbg.verbosePrefix(prefix(), CALL_INFO, 2, NIC_DBG_SEND_MACHINE, "get timing for packet %"
    PRIu64
    " size=%lu numPending=%d\n",
...
void Nic::SendMachine::InQ::ready(FireflyNetworkEvent *ev, int dest, Callback callback,
                                  uint64_t pktNum) {
    m_dbg.verbosePrefix(prefix(), CALL_INFO, 2, NIC_DBG_SEND_MACHINE, "packet %"
    PRIu64
    " is ready, expected=%"
    PRIu64
...
void Nic::SendMachine::InQ::ready2(FireflyNetworkEvent *ev, int dest, Callback callback) {
    m_dbg.verbosePrefix(prefix(), CALL_INFO, 2, NIC_DBG_SEND_MACHINE,
                        "pass packet to OutQ numBytes=%lu\n", ev->bufSize());
    --m_numPending;
...
void Nic::SendMachine::OutQ::enque(FireflyNetworkEvent *ev, int dest, Callback callback) {
    ...
    PriorityX *px = new PriorityX(m_lastEnq, m_enqCnt,
                                  new X(std::bind(&Nic::SendMachine::OutQ::pop, this, callback), ev,
                                        dest));

    ++m_qCnt;
    m_dbg.verbosePrefix(prefix(), CALL_INFO, 2, NIC_DBG_SEND_MACHINE, "qCnt=%d priority=%"
    PRIu64
