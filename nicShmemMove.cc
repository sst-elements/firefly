...
void Nic::ShmemSendMoveMem::copyOut(Output &dbg, int numBytes, FireflyNetworkEvent &event,
    ...
    dbg.debug(CALL_INFO, 3, NIC_DBG_SEND_MACHINE, "pktSpace=%lu dataLeft=%lu xferSize=%lu addr=%"
    PRIx64
    "\n",
...
void Nic::ShmemSendMoveValue::copyOut(Output &dbg, int numBytes, FireflyNetworkEvent &event,
                                      std::vector <MemOp> &vec) {
    size_t space = numBytes - event.bufSize();
    size_t len = m_value.getLength() > space ? space : m_value.getLength();

    dbg.debug(CALL_INFO, 3, NIC_DBG_SEND_MACHINE,
              "PacketSize=%d event.bufSpace()=%lu space=%lu len=%lu\n",
...
void Nic::ShmemSendMove2Value::copyOut(Output &dbg, int numBytes, FireflyNetworkEvent &event,
                                       std::vector <MemOp> &vec) {
    size_t space = numBytes - event.bufSize();
...
bool Nic::ShmemRecvMoveMem::copyIn(Output &dbg, FireflyNetworkEvent &event,
                                   std::vector <MemOp> &vec) {
    ...
    dbg.debug(CALL_INFO, 3, NIC_DBG_RECV_MOVE, "backing=%p addr=%#"
    PRIx64
...
bool Nic::ShmemRecvMoveMemOp::copyIn(Output &dbg, FireflyNetworkEvent &event,
                                     std::vector <MemOp> &vec) {
    size_t length = event.bufSize();
    dbg.debug(CALL_INFO, 3, NIC_DBG_RECV_MOVE, "event.bufSize()=%lu\n", event.bufSize());
    dbg.debug(CALL_INFO, 3, NIC_DBG_RECV_MOVE, "backing=%p addr=%#"
...
bool Nic::ShmemRecvMoveValue::copyIn(Output &dbg, FireflyNetworkEvent &event,
                                     std::vector <MemOp> &vec) {
    size_t length = event.bufSize();
    dbg.debug(CALL_INFO, 3, NIC_DBG_RECV_MOVE, "event.bufSize()=%lu\n", event.bufSize());
