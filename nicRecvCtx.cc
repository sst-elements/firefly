...
bool Nic::RecvMachine::Ctx::processPkt(FireflyNetworkEvent *ev) {
    ...
    m_dbg.verbosePrefix(prefix(), CALL_INFO, 2, NIC_DBG_RECV_CTX,
                        "got a network pkt from node=%d pid=%d stream=%d for pid=%d size=%zu\n",
...
bool Nic::RecvMachine::Ctx::processStdPkt(FireflyNetworkEvent *ev) {
    ...
    if (ev->isHdr()) {
        ...
        if (m_streamMap.find(srcKey) != m_streamMap.end()) {
            ...
            m_dbg.fatal(CALL_INFO, -1,
                        "m_streamMap.find(srcKey) != m_streamMap.end() node=%d pid=%d stream=%d for pid=%d\n",
                        ev->getSrcNode(), ev->getSrcPid(), ev->getSrcStream(), m_pid);
        }
        stream = newStream(ev);
        m_dbg.verbosePrefix(prefix(), CALL_INFO, 1, NIC_DBG_RECV_CTX,
                            "new stream %p %s node=%d pid=%d stream=%d for pid=%d\n", stream,
                            ev->isTail() ? "single packet stream" : "",
                            ev->getSrcNode(), ev->getSrcPid(), ev->getSrcStream(), m_pid);
    ...
    if (stream->isBlocked()) {
        m_dbg.verbosePrefix(prefix(), CALL_INFO, 2, NIC_DBG_RECV_CTX,
                            "stream is blocked stream=%p\n", stream);
        stream->setWakeup(
            [=]() {
                m_dbg.verbosePrefix(prefix(), CALL_INFO_LAMBDA, "processStdPkt", 2,
                                    NIC_DBG_RECV_CTX, "stream is unblocked stream=%p\n", stream);
                stream->processPkt(ev);
...
Nic::RecvMachine::StreamBase *Nic::RecvMachine::Ctx::newStream(FireflyNetworkEvent *ev) {
    ...
    switch (hdr.op) {
        case MsgHdr::Msg:
            return new MsgStream(m_dbg, this, ev->getSrcNode(), ev->getSrcPid(), ev->getDestPid(),
                                 ev);
            break;
        case MsgHdr::Rdma:
            return new RdmaStream(m_dbg, this, ev->getSrcNode(), ev->getSrcPid(), ev->getDestPid(),
                                  ev);
            break;
        case MsgHdr::Shmem:
            return new ShmemStream(m_dbg, this, ev->getSrcNode(), ev->getSrcPid(), ev->getDestPid(),
                                   ev);
            break;
    }
    assert(0);
...
Nic::EntryBase *Nic::RecvMachine::Ctx::findRecv(int srcNode, int srcPid, MsgHdr &hdr,
                                                MatchMsgHdr &matchHdr) {
    m_dbg.verbosePrefix(prefix(), CALL_INFO, 2, NIC_DBG_RECV_CTX,
                        "need a recv entry, srcNic=%d srcPid=%d "
...
Nic::SendEntryBase *Nic::RecvMachine::Ctx::findGet(int srcNode, int srcPid, RdmaMsgHdr &rdmaHdr) {
    m_dbg.verbosePrefix(prefix(), CALL_INFO, 1, NIC_DBG_RECV_CTX,
                        "srcNode=%d srcPid=%d rgnNum=%d offset=%d respKey=%d\n",
    ...
    return new PutOrgnEntry(m_pid, nic().getSendStreamNum(m_pid), srcNode, srcPid, rdmaHdr.respKey,
                            entry);
}

Nic::DmaRecvEntry *Nic::RecvMachine::Ctx::findPut(int srcNode, MsgHdr &hdr, RdmaMsgHdr &rdmahdr) {
    m_dbg.verbosePrefix(prefix(), CALL_INFO, 1, NIC_DBG_RECV_CTX,
                        "srcNode=%d rgnNum=%d offset=%d respKey=%d\n",
