...
public:
    Ctx(Output &output, RecvMachine &rm, int pid, int qsize) :
        ...
        m_prefix = "@t:" + std::to_string(rm.nic().getNodeId()) + ":Nic::RecvMachine::Ctx" +
                   std::to_string(pid) + "::@p():@l ";
    }
    ...
    void needRecv(StreamBase *stream) {
        ...
        m_dbg.verbosePrefix(prefix(), CALL_INFO, 2, NIC_DBG_RECV_CTX,
                            "pid%d blocked, srcNode=%d srcPid=%d\n",
                            stream->getSrcPid(), stream->getSrcNode(), stream->getSrcPid());

        m_rm.m_nic.notifyNeedRecv(stream->getMyPid(), stream->getSrcNode(), stream->getSrcPid(),
                                  stream->length());
    }
    ...
    void deleteStream(StreamBase *stream) {
        m_rm.nic().schedCallback([=]() {
                                     ...
                                     m_dbg.verbosePrefix(prefix(), CALL_INFO_LAMBDA, "deleteStream", 1, NIC_DBG_RECV_CTX,
                                                         "%p\n", stream);
