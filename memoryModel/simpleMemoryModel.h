...
#include "detailedUnit.h"
    ...
    public:
        void init(int _slot, Work *_work = NULL) {
            callback = NULL;
            unit = NULL;
            slot = _slot;
            work = _work;
        ...
        void init(Callback *_callback) {
            callback = _callback;
            unit = NULL;
            work = NULL;
        }

        void init(UnitBase *_unit, UnitBase *_srcUnit = NULL) {
            callback = NULL;
            unit = _unit;
            srcUnit = _srcUnit;
            work = NULL;
...
public:
    ...
    SimpleMemoryModel(Component *comp, Params &params) :
        ...
        m_dbg.init(buffer, params.find<uint32_t>("verboseLevel", 0),
                   params.find<uint32_t>("verboseMask", -1), Output::STDOUT);
        ...
        if (0 == params.find<std::string>("printConfig", "no").compare("yes")) {
            m_dbg.output(
                "Node id=%d is using SimpleMemoryModel, useBusBridge=%d, useHostCache=%d\n", id,
                useBusBridge, useHostCache);
        }
        ...
        MuxUnit *nicMuxUnit;
        if (useHostCache) {
            m_hostCacheUnit = new CacheUnit(*this, m_dbg, id, m_memUnit, hostCacheUnitSize,
                                            hostCacheLineSize, hostCacheNumMSHR, "host");
        ...
        if (useBusBridge) {
            ...
            m_busBridgeUnit = new BusBridgeUnit(*this, m_dbg, id, m_muxUnit, busBandwidth,
                                                busNumLinks, busLatency,
                                                TLP_overhead, DLL_bytes, hostCacheLineSize,
                                                widgetSlots);
        ...
        m_sharedTlb = new SharedTlb(*this, m_dbg, id, tlbSize, tlbPageSize, tlbMissLat_ns,
                                    numWalkers);
        ...
        for (int i = 0; i < numCores; i++) {
            ...
            m_threads.push_back(
                new Thread(*this, "host", m_dbg, id, i, 64,
                           new LoadUnit(*this, m_dbg, id, i, m_muxUnit, hostNumLoadSlots,
                                        "host_thread"),
                           new StoreUnit(*this, m_dbg, id, i, m_muxUnit, hostNumStoreSlots,
                                         "host_thread")
                )
            );
        }
        ...
        m_selfLink = comp->configureSelfLink("Nic::SimpleMemoryModel", "1 ns",
                                             new Event::Handler<SimpleMemoryModel>(this,
                                                                                   &SimpleMemoryModel::handleSelfEvent));
    }
    ...
    virtual void schedHostCallback(int core, std::vector <MemOp> *ops, Callback callback) {
        SimTime_t now = getCurrentSimTimeNano();
        m_dbg.debug(CALL_INFO, 3, SM_MASK, "now=%"
        PRIu64
        "\n", now );
