...
struct WidgetEntry {
    WidgetEntry(int cacheLineSize, MemReq *req, SimTime_t issueTime, Callback *callback = NULL)
        : curAccess(0),
...
public:
    ...
    bool load(UnitBase *src, MemReq *req, Callback *callback) {
        m_dbg.verbosePrefix(prefix(), CALL_INFO, 1, BUS_WIDGET_MASK, "addr=%#"
        PRIx64
        " length=%lu pending=%d\n",
    ...
    void process() {
        ...
        if (entry.isDone()) {
            ...
            *callback = [=]() {
                ...
                --m_numPending;
                m_dbg.verbosePrefix(prefix(), CALL_INFO_LAMBDA, "process", 1, BUS_WIDGET_MASK,
                                    "addr=%#"
                PRIx64
                " complete, latency=%"
                PRIu64
...
public:
    BusStoreWidget(SimpleMemoryModel &model, Output &dbg, int id, Unit *cache, int width, int qSize,
                   int latency) :
        Unit(model, dbg), m_cache(cache), m_width(width), m_qSize(qSize), m_latency(latency),
        m_blocked(false), m_blockedSrc(NULL), m_scheduled(false), m_numPending(0),
        m_name("BusStoreWidget") {
    ...
    bool storeCB(UnitBase *src, MemReq *req, Callback *callback = NULL) {
        ...
        WidgetEntry *entry = new WidgetEntry(m_width, req, m_model.getCurrentSimTimeNano(),
                                             callback);
        m_dbg.verbosePrefix(prefix(), CALL_INFO, 1, BUS_WIDGET_MASK, "addr=%#"
        PRIx64
