...
public:
    CacheUnit(SimpleMemoryModel &model, Output &dbg, int id, Unit *memory, int cacheSize,
        ...
        Unit(model, dbg), m_memory(memory), m_numPending(0), m_blockedSrc(NULL), m_numMSHR(numMSHR),
        m_scheduled(false),
        m_cacheLineSize(cacheLineSize), m_qSize(numMSHR), m_numIssuedLoads(0),
        m_cache(cacheSize), m_blockedOnMemUnit(false) {
        m_prefix =
            "@t:" + std::to_string(id) + ":SimpleMemoryModel::" + name + "CacheUnit::@p():@l ";
    ...
    bool load(UnitBase *src, MemReq *req, Callback *callback) {
        ...
        //assert( (req->addr & (m_cacheLineSize - 1) ) == 0 );
        return addEntry(
            new Entry(Entry::Load, src, req, m_model.getCurrentSimTimeNano(), callback));
    }

    void resume(UnitBase *src = NULL) {
        m_blockedOnMemUnit = false;
        m_dbg.verbosePrefix(prefix(), CALL_INFO, 1, CACHE_MASK,
                            "blocked=%lu bockedDone=%lu numPending=%d\n",
        ...
        while (!m_blockedOnMemUnit && !m_blockedDone.empty()) {
            ...
            m_blockedDone.pop();
            m_dbg.verbosePrefix(prefix(), CALL_INFO, 1, CACHE_MASK, "process blocked done, addr=%#"
            PRIx64
            "\n", entry->addr);
            loadDone2(entry);
        }
...
private:
    ...
    bool addEntry(Entry *entry) {
        ...
        m_dbg.verbosePrefix(prefix(), CALL_INFO, 1, CACHE_MASK, "%s addr=%#"
        PRIx64
        " pending=%d\n",
    ...
    void checkHitNew(Entry *entry) {
        ...
        if (checkHit(entry, false)) {
            m_dbg.verbosePrefix(prefix(), CALL_INFO, 2, CACHE_MASK, "queue blocked %s addr=%#"
            PRIx64
    ...
    bool checkHit(Entry *entry, bool flag) {
        ...
        m_dbg.verbosePrefix(prefix(), CALL_INFO, 2, CACHE_MASK, "%s addr=%#"
        PRIx64
        "\n", entry->op == Entry::Load ? "Load" : "Store", entry->req->addr);
        if (m_cache.isValid(entry->req->addr)) {
            m_totalCnt->addData(1);
            m_dbg.verbosePrefix(prefix(), CALL_INFO, 1, CACHE_MASK, "done, hit %s addr=%#"
            PRIx64
            "\n",
                entry->op == Entry::Load ? "Load" : "Store", entry->req->addr);
            m_hitCnt->addData(1);
            ...
            m_totalCnt->addData(1);
            m_dbg.verbosePrefix(prefix(), CALL_INFO, 2, CACHE_MASK, "pending addr=%#"
            PRIx64
            "\n", entry->req->addr);
            ...
            if (!blocked() && (m_blockedQ.empty() || flag)) {
                ...
                m_dbg.verbosePrefix(prefix(), CALL_INFO, 2, CACHE_MASK, "blocked addr=%#"
                PRIx64
                "\n", entry->req->addr);
                return true;
            }
    ...
    void miss(Entry *entry) {
        ...
        m_dbg.verbosePrefix(prefix(), CALL_INFO, 2, CACHE_MASK, "addr=%#"
        PRIx64
        "\n", entry->req->addr);
    ...
    void loadDone2(Entry *entry) {
        ...
        m_dbg.verbosePrefix(prefix(), CALL_INFO, 1, CACHE_MASK, "done, op=%s addr=%#"
        PRIx64
        " latency=%"
        PRIu64
        " numIssued=%d\n",
        ...
        if (m_pendingMap[addr]) {
            while (!m_pendingMap[addr]->empty()) {
                m_dbg.verbosePrefix(prefix(), CALL_INFO, 1, CACHE_MASK, "done addr=%#"
                PRIx64
                "\n", addr);
                decNumPending();
        ...
        m_dbg.verbosePrefix(prefix(), CALL_INFO, 2, CACHE_MASK, "insert addr=%#"
        PRIx64
        "\n", addr);
        m_cache.insert(addr);
    ...
    std::map<Hermes::Vaddr, std::queue < Entry * >* >
    m_pendingMap;
