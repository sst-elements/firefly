...
public:
    MemUnit(SimpleMemoryModel &model, Output &dbg, int id, int readLat_ns, int writeLat_ns,
            int numSlots) :
        Unit(model, dbg), m_pending(0), m_readLat_ns(readLat_ns), m_writeLat_ns(writeLat_ns),
        m_numSlots(numSlots) {
    ...
    bool store(UnitBase *src, MemReq *req) {
        m_dbg.verbosePrefix(prefix(), CALL_INFO, 1, MEM_MASK, "addr=%#"
        PRIx64
        " length=%lu\n", req->addr, req->length);
        m_stores->addData(1);
...
private:
    ...
    struct Entry {
        ...
        Entry(SimTime_t delay, Op op, MemReq *memReq, UnitBase *src, Callback *callback,
              SimTime_t qTime) :
    ...
    bool work(SimTime_t delay, Op op, MemReq *req, UnitBase *src, SimTime_t qTime,
        ...
        *cb = [=]() {
            ...
            m_dbg.verbosePrefix(prefix(), CALL_INFO, 1, MEM_MASK, "%s complete latency=%"
            PRIu64
            " qLatency=%"
            PRIu64
            " addr=%#"
