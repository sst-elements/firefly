...
class BusBridgeUnit : public Unit {
    ...
    struct Entry {
        Entry(UnitBase *src, MemReq *req, Callback *callback = NULL) : src(src), req(req),
                                                                       callback(callback),
                                                                       addr(req->addr),
                                                                       length(req->length) {}
...
public:
    BusBridgeUnit(SimpleMemoryModel &model, Output &dbg, int id, Unit *cache, double bandwidth,
                  int numLinks,
                  int latency, int TLP_overhead, int dll_bytes, int cacheLineSize, int widgetSlots)
        :
        Unit(model, dbg), m_cache(cache), m_bandwidth_GB(bandwidth), m_numLinks(numLinks),
        m_blocked(2, {NULL, 0}),
        m_TLP_overhead(TLP_overhead), m_DLL_bytes(dll_bytes), m_cacheLineSize(cacheLineSize),
        m_latency(latency),
        m_reqBus(*this, id, "Req", true,
                 std::bind(&BusBridgeUnit::processReq, this, std::placeholders::_1)),
        m_respBus(*this, id, "Resp", false,
                  std::bind(&BusBridgeUnit::processResp, this, std::placeholders::_1)) {
    ...
    bool load(UnitBase *src, MemReq *req, Callback *callback) {
        ...
        m_dbg.verbosePrefix(prefix(), CALL_INFO, 1, BUS_BRIDGE_MASK, "src=%p entry=%p addr=%#"
        PRIx64
    ...
    bool write(UnitBase *src, MemReq *req, Callback *callback) {
        ...
        m_dbg.verbosePrefix(prefix(), CALL_INFO, 1, BUS_BRIDGE_MASK, "entry=%p addr=%#"
        PRIx64
        " length=%lu %s %p\n",
...
private:
    ...
    public:
        Bus(BusBridgeUnit &unit, int id, std::string name, bool isReq,
            std::function<void(Entry *)> func) :
            m_unit(unit), isReq(isReq), busy(false), m_processEntry(func) {
            m_prefix = "@t:" + std::to_string(id) + ":SimpleMemoryModel::BusBridgeUnit::" + name +
                       "::@p():@l ";
        }

        void addReq(Entry *req) {
            m_pendingReqQ.push(req);
            m_unit.m_dbg.verbosePrefix(prefix(), CALL_INFO, 2, BUS_BRIDGE_MASK, "src=%p %s %#"
            PRIx64
            " q size=%lu\n",
                    ...
                    *cb = std::bind(&Bus::reqArrived, this, (Entry *) NULL);
                    ...
                    if (isReq) {
                        if (entry->callback) {
                            ...
                            delay = m_unit.calcByteDelay(
                                entry->req->length + m_unit.TLP_overhead());
                        }
                    } else {
                    ...
                    m_unit.m_dbg.verbosePrefix(prefix(), CALL_INFO, 2, BUS_BRIDGE_MASK,
                                               "entry=%p addr=%#"
                    PRIx64
                    " length=%lu delay=%"
                    PRIu64
    ...
    void processReq(Entry *entry) {
        ...
        SimTime_t now = m_model.getCurrentSimTimeNano();
        m_dbg.verbosePrefix(prefix(), CALL_INFO, 2, BUS_BRIDGE_MASK, "entry=%p qd_latency=%"
        PRIu64
        " xmit_latency=%"
        PRIu64
        "\n",
        ...
        if (entry->callback) {
            ...
            Callback *cb = new Callback;
            *cb = [=]() {
                m_dbg.verbosePrefix(prefix(), CALL_INFO_LAMBDA, "processReq", 1, BUS_BRIDGE_MASK,
                                    "load done entry=%p addr=%#"
                PRIx64
                " length=%lu latency=%"
                PRIu64
                "\n",
