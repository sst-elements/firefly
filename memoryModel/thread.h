...
public:
    ...
    void print(Output &dbg, const char *prefix) {
        for (unsigned i = 0; i < m_ops->size(); i++) {
            dbg.verbosePrefix(prefix, CALL_INFO, 2, THREAD_MASK, "%s %#"
            PRIx64
...
public:
    Thread(SimpleMemoryModel &model, std::string name, Output &output, int id, int thread_id,
           int accessSize, Unit *load, Unit *store) :
        m_model(model), m_name(name), m_dbg(output), m_id(id), m_loadUnit(load), m_storeUnit(store),
        m_maxAccessSize(accessSize), m_nextOp(NULL), m_waitingOnOp(NULL), m_blocked(false),
        m_curWorkNum(0), m_lastDelete(0) {
        m_prefix = "@t:" + std::to_string(id) + ":SimpleMemoryModel::" + name + "::@p():@l ";
        m_dbg.verbosePrefix(prefix(), CALL_INFO, 1, THREAD_MASK, "this=%p\n", this);
        m_workQdepth = model.registerStatistic<uint64_t>(name + "_thread_work_Q_depth",
    ...
    void printStatus(Output &out, int id) {
        out.output("NIC %d: %s cur=%d last=%d blocked=%d %p %p\n", id, m_name.c_str(), m_curWorkNum,
                   m_lastDelete, m_blocked, m_nextOp, m_waitingOnOp);
        if (m_workQ.size()) {
    ...
    void addWork(Work *work) {
        m_dbg.verbosePrefix(prefix(), CALL_INFO, 1, THREAD_MASK,
                            "work=%p numOps=%lu workSize=%lu blocked=%d\n",
...
private:
    ...
    void process(MemOp *op = NULL) {
        ...
        m_dbg.verbosePrefix(prefix(), CALL_INFO, 2, THREAD_MASK,
                            "op=%s op.length=%lu offset=%lu addr=%#"
        PRIx64
        " length=%lu\n",
