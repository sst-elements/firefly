...
public:
    SharedTlb(SimpleMemoryModel &model, Output &dbg, int id, int size, int pageSize,
              int tlbMissLat_ns, int numWalkers) :
        m_model(model), m_dbg(dbg), m_tlbMissLat_ns(tlbMissLat_ns), m_numWalkers(numWalkers),
        ...
        m_dbg.verbosePrefix(prefix(), CALL_INFO, 1, SHARED_TLB_MASK, "tlbSize=%d, pageMask=%#"
        PRIx64
        ", numWalkers=%d\n",
    ...
    uint64_t lookup(MemReq *req, Callback callback, bool flag = false) {
        ...
        if (m_cache.isValid(pageAddr)) {
            m_dbg.verbosePrefix(prefix(), CALL_INFO, 1, SHARED_TLB_MASK, "Hit: virtAddr=%#"
            PRIx64
            " physAddr=%#"
            PRIx64
            " pageAddr=%#"
            ...
            if (m_pendingMap.find(pageAddr) != m_pendingMap.end()) {
                m_dbg.verbosePrefix(prefix(), CALL_INFO, 1, SHARED_TLB_MASK, "Pending: virtAddr=%#"
                PRIx64
                " physAddr=%#"
                PRIx64
                " pageAddr=%#"
...
private:
    ...
    void resolved(MemReq *req, Callback callback) {
        ...
        while (!m_pendingLookups.empty()) {
            ...
            if (m_cache.isValid(pageAddr)) {
                ...
                *cb = std::bind(&SharedTlb::resolved, this, req, callback);
                m_model.schedCallback(m_tlbMissLat_ns, cb);
                m_dbg.verbosePrefix(prefix(), CALL_INFO, 1, SHARED_TLB_MASK, "Schedule: virtAddr=%#"
                PRIx64
                " physAddr=%#"
                PRIx64
                " pageAddr=%#"
    ...
    std::unordered_map <uint64_t, std::queue<std::pair < MemReq * , Callback>> >
    m_pendingMap;
