...
class NWayCache {
public:
    NWayCache(int assoc, uint32_t nSets, int pageSize) : m_pageShift(0), m_setShift(0),
                                                         m_setMask(nSets - 1) {
    ...
    void printStats(Output &output) {
        for (unsigned i = 0; i < m_stats.size(); i++) {
            if (m_stats[i].first) {
                output.output("set=%d: total=%"
                PRIi64
                " %f hits\n", i, m_stats[i].first, (float) m_stats[i].second /
                                                   (float) m_stats[i].first );
            }
        }
    }
