...
class Tlb : public Unit {
    ...
    struct Entry {
        ...
        Entry(MemReq *req, Callback *callback, Type type) : req(req), callback(callback),
                                                            type(type) {}
...
public:
    Tlb(SimpleMemoryModel &model, Output &dbg, int id, std::string name, Unit *load, Unit *store,
        int size, int pageSize, int tlbMissLat_ns,
        int numWalkers, int maxStores, int maxLoads) :
        Unit(model, dbg), m_curPid(-1), m_cache(size), m_cacheSize(size),
        ...
        m_dbg.verbosePrefix(prefix(), CALL_INFO, 1, TLB_MASK, "tlbSize=%d, pageMask=%#"
        PRIx64
        ", numWalkers=%d\n",
    ...
    ~Tlb() {
        if (0 && m_total) {
            m_dbg.output("%s total requests %"
            PRIu64
            " %f percent hits,  %f percent flushes\n",
                stats.c_str(), m_total, (float) m_hitCnt / (float) m_total, (float) m_flush /
                                                                            (float) m_total);
        }
    }
    ...
    bool storeCB(UnitBase *src, MemReq *req, Callback *callback) {
        ...
        m_dbg.verbosePrefix(prefix(), CALL_INFO, 1, TLB_MASK, "req Addr %#"
        PRIx64
        ", page Addr %#"
        PRIx64
        "\n", addr, req->addr );
...
private:
    ...
    bool tlbMiss2(Entry *entry) {
        ...
        m_dbg.verbosePrefix(prefix(), CALL_INFO, 1, TLB_MASK, "addr=%#"
        PRIx64
        "\n", addr);

        bool retval = false;
        if (m_pendingWalkList.find(addr) != m_pendingWalkList.end()) {
            m_dbg.verbosePrefix(prefix(), CALL_INFO, 1, TLB_MASK, "add pending, page addr=%#"
            PRIx64
            "\n", addr);
            m_pendingWalkList[addr].push(entry);
        } else if (m_pendingWalks < m_numWalkers) {
            m_dbg.verbosePrefix(prefix(), CALL_INFO, 1, TLB_MASK, "walk, page addr=%#"
            PRIx64
            "\n", addr);
    ...
    void processMissComplete(uint64_t addr) {
        m_dbg.verbosePrefix(prefix(), CALL_INFO, 1, TLB_MASK, "page addr=%#"
        PRIx64
        "\n", addr);
        --m_pendingWalks;
    ...
    void walk(int pid, uint64_t addr, Callback callback) {
        ...
        m_dbg.verbosePrefix(prefix(), CALL_INFO, 1, TLB_MASK, "pid=%d addr=%#"
        PRIx64
        " evictAddr=%#"
        PRIx64
        "\n", pid, addr, evictAddr );
