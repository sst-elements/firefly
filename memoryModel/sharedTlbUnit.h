...
class SharedTlbUnit : public Unit {
    ...
    std::string m_name;
public:
    SharedTlbUnit(SimpleMemoryModel &model, Output &dbg, int id, std::string name, SharedTlb *tlb,
                  Unit *load, Unit *store, int maxStores, int maxLoads) :
        Unit(model, dbg),
        m_name(name),
        m_tlb(tlb),
        m_load(load),
        ...
        m_prefix =
            "@t:" + std::to_string(id) + ":SimpleMemoryModel::" + name + "SharedTlbUnit::@p():@l ";
    }
    ...
    void printStatus(Output &out, int id) {
        out.output("NIC %d: %s pending=%d %p %p\n", id, m_name.c_str(), m_pendingLookups,
                   m_blockedStoreSrc, m_blockedLoadSrc);
    }
    ...
    bool storeCB(UnitBase *src, MemReq *req, Callback *callback) {
        ...
        uint64_t addr = m_tlb->lookup(req,
                                      std::bind(&SharedTlbUnit::storeAddrResolved, this, callback,
                                                std::placeholders::_1, std::placeholders::_2));
        if (-1 == addr) {
            ++m_pendingLookups;
            m_dbg.verbosePrefix(prefix(), CALL_INFO, 1, TLB_MASK, "Miss, pid %d, req Addr %#"
            PRIx64
            " pendingLookups=%d\n",
                req->pid, req->addr, m_pendingLookups );
        } else {
            m_dbg.verbosePrefix(prefix(), CALL_INFO, 1, TLB_MASK, "Hit, pid %d, req Addr %#"
            PRIx64
            "\n", req->pid, req->addr );
        ...
        if (blockedStore()) {
            m_dbg.verbosePrefix(prefix(), CALL_INFO, 1, TLB_MASK,
                                "Blocking source, pid %d, req Addr %#"
            PRIx64
            "\n", req->pid, req->addr );
    ...
    bool load(UnitBase *src, MemReq *req, Callback *callback) {
        ...
        uint64_t addr = m_tlb->lookup(req,
                                      std::bind(&SharedTlbUnit::loadAddrResolved, this, callback,
                                                std::placeholders::_1, std::placeholders::_2));
        if (-1 == addr) {
            ++m_pendingLookups;
            m_dbg.verbosePrefix(prefix(), CALL_INFO, 1, TLB_MASK, "Miss, pid %d, req Addr %#"
...
private:
    bool passUpLoad(Entry *entry) {
        ...
        PRIx64
        "\n", entry->req->addr);
        m_dbg.verbosePrefix(prefix(), CALL_INFO, 1, TLB_MASK, "call load %#"
        PRIx64
        "\n", entry->req->addr );
    ...
    bool passUpStore(Entry *entry) {
        ...
        PRIx64
        "\n", entry->req->addr);
        m_dbg.verbosePrefix(prefix(), CALL_INFO, 1, TLB_MASK, "call storeCB %#"
        PRIx64
        "\n", entry->req->addr );
