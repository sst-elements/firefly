...
class LoadUnit : public Unit {
    struct Entry {
        Entry(MemReq *req, Callback *callback, SimTime_t time) : req(req), callback(callback),
                                                                 postTime(time) {}
...
public:
    LoadUnit(SimpleMemoryModel &model, Output &dbg, int id, int thread_id, Unit *cache,
        ...
        m_blockedSrc(NULL), m_numPending(0)//, m_name(name)
    {
        ...
        m_prefix =
            "@t:" + std::to_string(id) + ":SimpleMemoryModel::" + name + "LoadUnit::@p():@l ";
        m_dbg.verbosePrefix(prefix(), CALL_INFO, 1, LOAD_MASK, "maxPending=%d\n", m_qSize);
        m_pendingQdepth = model.registerStatistic<uint64_t>(name + "_load_pending_Q_depth",
                                                            std::to_string(thread_id));
        m_latency = model.registerStatistic<uint64_t>(name + "_load_latency",
                                                      std::to_string(thread_id));
    }
    ...
    bool load(UnitBase *src, MemReq *req, Callback *callback) {
        ...
        m_dbg.verbosePrefix(prefix(), CALL_INFO, 1, LOAD_MASK, "addr=%#"
        PRIx64
        " length=%lu pending=%lu\n", req->addr, req->length, m_pendingQ.size());
...
private:
    void process() {
        ...
        m_dbg.verbosePrefix(prefix(), CALL_INFO, 3, LOAD_MASK, "addr=%#"
        PRIx64
        ...
        *cb = [=]() {
            ...
            m_dbg.verbosePrefix(prefix(), CALL_INFO_LAMBDA, "process", 1, LOAD_MASK,
                                " complete, latency=%"
            PRIu64
            " addr=%#"
            PRIx64
            ...
            --m_numPending;
            if (entry.callback) {
                m_dbg.verbosePrefix(prefix(), CALL_INFO_LAMBDA, "process", 3, LOAD_MASK,
                                    "tell src load is complete\n");
                m_model.schedCallback(0, entry.callback);
            }

            if (m_blockedSrc) {
                m_dbg.verbosePrefix(prefix(), CALL_INFO_LAMBDA, "process", 2, LOAD_MASK,
                                    "unblock src\n");
