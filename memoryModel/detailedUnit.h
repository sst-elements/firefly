...
public:
    DetailedUnit(SimpleMemoryModel &model, Output &dbg, int id, Params &params) :
        ...
        m_mem_link = dynamic_cast<Interfaces::SimpleMem *>( model.loadSubComponent(name,
                                                                                   detailedParams));
        ...
        bool init_success = m_mem_link->initialize("detailed",
                                                   new Interfaces::SimpleMem::Handler<DetailedUnit>(
                                                       this, &DetailedUnit::handleEvent));

        if (init_success) {
            dbg.verbose(CALL_INFO, 1, DETAILED_MASK,
                        "Loaded memory initialize routine returned successfully.\n");
        } else {
    ...
    bool store(UnitBase *src, MemReq *req) {
        m_dbg.verbosePrefix(prefix(), CALL_INFO, 1, DETAILED_MASK, "addr=%#"
        PRIx64
        " length=%lu\n", req->addr, req->length);
...
private:
    ...
    void handleEvent(Interfaces::SimpleMem::Request *ev) {
        ...
        std::map<Interfaces::SimpleMem::Request::id_t, Entry *>::iterator reqFind = m_inflight.find(
            reqID);

        if (reqFind == m_inflight.end()) {
            m_dbg.fatal(CALL_INFO, -1, "Unable to find request %"
            PRIu64
            " in request map.\n", reqID);
        } else {
            ...
            m_dbg.verbosePrefix(prefix(), CALL_INFO, 1, DETAILED_MASK, "id=%"
            PRIu64
            ", inflight=%zu blockedSrc=%zu addr=%"
            PRIx64
            " pendingQ=%zu\n",
        ...
        if (!m_pendingQ.empty()) {
            ...
            if (m_requestsPending[entry->op] < m_maxRequestsPending[entry->op]) {
                ...
                req = new Interfaces::SimpleMem::Request(
                    entry->op == Read ? Interfaces::SimpleMem::Request::Read
