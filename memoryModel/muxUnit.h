...
public:
    ...
    MuxUnit(SimpleMemoryModel &model, Output &dbg, int id, Unit *unit, std::string name) : Unit(
        model, dbg), m_unit(unit), m_blockedSrc(NULL), m_scheduled(false) {
    ...
    bool store(UnitBase *src, MemReq *req) {
        m_dbg.verbosePrefix(prefix(), CALL_INFO, 1, MUX_MASK, "%s addr=%#"
        PRIx64
    ...
    bool load(UnitBase *src, MemReq *req, Callback *callback) {
        ...
        if (!m_blockedSrc && !m_scheduled) {
            ...
            *cb = [=]() {
                m_dbg.verbosePrefix(prefix(), CALL_INFO_LAMBDA, "load", 1, MUX_MASK,
                                    "load done latency=%"
                PRIu64
                "\n",
            ...
            m_dbg.verbosePrefix(prefix(), CALL_INFO, 1, MUX_MASK, "blocking\n");
            m_blockedQ.push(
                Entry(Entry::Load, src, req, m_model.getCurrentSimTimeNano(), callback));
            return true;
        }
    }
    ...
    void processQ() {
        ...
        if (Entry::Load == entry.op) {
            Callback *cb = new Callback;
            *cb = [=]() {
                m_dbg.verbosePrefix(prefix(), CALL_INFO_LAMBDA, "processQ", 1, MUX_MASK,
                                    "load done latency=%"
                PRIu64
                "\n",
                    m_model.getCurrentSimTimeNano() - now );
