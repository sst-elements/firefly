...
public:
    StoreUnit(SimpleMemoryModel &model, Output &dbg, int id, int thread_id, Unit *cache,
        ...
        Unit(model, dbg), m_qSize(numSlots), m_cache(cache), m_blocked(false), m_blockedSrc(NULL),
        ...
        m_pendingQdepth = model.registerStatistic<uint64_t>(name + "_store_pending_Q_depth",
    ...
    bool storeCB(UnitBase *src, MemReq *req, Callback *callback = NULL) {
        ...
        m_dbg.verbosePrefix(prefix(), CALL_INFO, 1, STORE_MASK, "addr=%#"
        PRIx64
