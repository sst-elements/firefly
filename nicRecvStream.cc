...
Nic::RecvMachine::StreamBase::StreamBase(Output &output, Ctx *ctx, int srcNode, int srcPid,
                                         int myPid) :
    m_dbg(output), m_ctx(ctx), m_unit(-1), m_srcNode(srcNode), m_srcPid(srcPid), m_myPid(myPid),
    m_recvEntry(NULL), m_sendEntry(NULL), m_numPending(0), m_pktNum(0), m_expectedPkt(0),
    m_blockedNeedRecv(NULL) {
    m_prefix =
        "@t:" + std::to_string(ctx->nic().getNodeId()) + ":Nic::RecvMachine::StreamBase::@p():@l ";
...
Nic::RecvMachine::StreamBase::~StreamBase() {
    ...
    m_dbg.verbosePrefix(prefix(), CALL_INFO, 1, NIC_DBG_RECV_STREAM, "this=%p bytes=%zu latency=%"
    PRIu64
    "\n", this,
        totalBytes, m_ctx->nic().getCurrentSimTimeNano() - m_start);

    if (m_sendEntry) {
        m_dbg.verbosePrefix(prefix(), CALL_INFO, 1, NIC_DBG_RECV_STREAM,
                            "core=%d targetNode=%d targetCore=%d\n",
...
void Nic::RecvMachine::StreamBase::processPktBody(FireflyNetworkEvent *ev) {
    ...
    m_dbg.verbosePrefix(prefix(), CALL_INFO, 2, NIC_DBG_RECV_STREAM, "get timing for packet %"
    PRIu64
    " size=%lu\n",
        m_pktNum, ev->bufSize());
    ...
    m_ctx->nic().dmaWrite(m_unit, m_myPid, vec,
                          std::bind(&Nic::RecvMachine::StreamBase::ready, this, finished,
                                    m_pktNum++));
}

void Nic::RecvMachine::StreamBase::ready(bool finished, uint64_t pktNum) {
    m_dbg.verbosePrefix(prefix(), CALL_INFO, 2, NIC_DBG_RECV_STREAM, "packet %"
    PRIu64
    " is ready\n", pktNum);
