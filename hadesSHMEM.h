...
namespace SST {
    namespace Firefly {
        ...
        private:
            ...
            public:
                ...
                void *findBacking(Hermes::Vaddr addr) {
                    ...
                    for (; iter != m_map.end(); ++iter) {
                        Entry &entry = iter->second;
                        if (addr >= entry.addr.getSimVAddr() &&
                            addr < entry.addr.getSimVAddr() + entry.length) {
            ...
            struct Broadcast : public Base {
                Broadcast(Vaddr dest, Vaddr source, size_t nelems, int root, int PE_start,
                    ...
                    Base(callback), dest(dest), source(source), nelems(nelems), root(root),
                    PE_start(PE_start),
            ...
            struct Alltoalls : public Base {
                Alltoalls(Vaddr dest, Vaddr source, int dst, int sst, size_t nelems, int elsize,
                          int PE_start, int logPE_stride, int PE_size, Vaddr pSync,
                          Shmem::Callback callback) :
                    Base(callback), dest(dest), source(source), dst(dst), sst(sst), nelems(nelems),
                    elsize(elsize), PE_start(PE_start), logPE_stride(logPE_stride),
                    PE_size(PE_size),
                    pSync(pSync) {}
            ...
            struct Reduction : public Base {
                Reduction(Vaddr dest, Vaddr source, int nelems, int PE_start, int logPE_stride,
                          int PE_size, Vaddr pSync, Shmem::ReduOp op, Hermes::Value::Type type,
                          Shmem::Callback callback) :
                    Base(callback), dest(dest), source(source), nelems(nelems), PE_start(PE_start),
                    logPE_stride(logPE_stride), PE_size(PE_size), pSync(pSync), op(op),
                    type(type) {}
            ...
            struct Get : public Base {
                Get(Vaddr dest, Vaddr src, size_t nelems, int pe, bool blocking,
                    Shmem::Callback callback, Shmem::Callback fini) :
                    Base(callback), dest(dest), src(src), nelems(nelems), pe(pe),
                    blocking(blocking), finiCallback(fini) {}
            ...
            struct Put : public Base {
                Put(Vaddr dest, Vaddr src, size_t nelems, int pe, bool blocking,
                    Shmem::Callback callback, Shmem::Callback fini) :
                    Base(callback), dest(dest), src(src), nelems(nelems), pe(pe),
                    blocking(blocking), finiCallback(fini) {}
            ...
            struct PutOp : public Base {
                PutOp(Hermes::Vaddr dest, Hermes::Vaddr src, size_t nelems, int pe,
                    ...
                    :
                    Base(callback), dest(dest), src(src), nelems(nelems), pe(pe), op(op),
                    type(type) {}
            ...
            struct Cswap : public Base {
                Cswap(Value &result, Vaddr addr, Value &cond, Value &value, int pe,
                      Shmem::Callback callback) :
            ...
            struct Fam_Get : public Base {
                Fam_Get(Hermes::Vaddr dest, Shmem::Fam_Descriptor rd, uint64_t offset,
                        uint64_t nbytes,
                        bool blocking, Shmem::Callback callback) :
                    Base(callback), dest(dest), rd(rd), offset(offset), nbytes(nbytes),
                    blocking(blocking) {}
        ...
        public:
            ...
            virtual void get(Hermes::Vaddr dest, Hermes::Vaddr src, size_t nelems, int pe,
                             bool blocking, Shmem::Callback);

            virtual void putv(Hermes::Vaddr dest, Hermes::Value &, int pe, Shmem::Callback);

            virtual void put(Hermes::Vaddr dest, Hermes::Vaddr src, size_t nelems, int pe,
                             bool blocking, Shmem::Callback);

            virtual void putOp(Hermes::Vaddr dest, Hermes::Vaddr src, size_t nelems, int pe,
                               Hermes::Shmem::ReduOp, Hermes::Value::Type, Shmem::Callback);

            virtual void wait_until(Hermes::Vaddr src, Hermes::Shmem::WaitOp, Hermes::Value &,
                                    Shmem::Callback);

            virtual void cswap(Hermes::Value &result, Hermes::Vaddr, Hermes::Value &cond,
                               Hermes::Value &value, int pe, Shmem::Callback);

            virtual void swap(Hermes::Value &result, Hermes::Vaddr, Hermes::Value &value, int pe,
                              Shmem::Callback);

            virtual void add(Hermes::Vaddr, Hermes::Value &, int pe, Shmem::Callback);

            virtual void fadd(Hermes::Value &, Hermes::Vaddr, Hermes::Value &, int pe,
                              Shmem::Callback);

            virtual void fam_add(Shmem::Fam_Descriptor fd, uint64_t, Hermes::Value &,
                                 Shmem::Callback &);

            virtual void fam_cswap(Hermes::Value &result, Shmem::Fam_Descriptor fd, uint64_t,
                                   Hermes::Value &oldValue, Hermes::Value &newValue,
                                   Shmem::Callback);

            virtual void fam_get(Hermes::Vaddr dest, Shmem::Fam_Descriptor fd, uint64_t offset,
                                 uint64_t nbytes,
                                 bool blocking, Shmem::Callback &);

            virtual void fam_put(Shmem::Fam_Descriptor fd, uint64_t offset, Hermes::Vaddr src,
                                 uint64_t nbytes,
                                 bool blocking, Shmem::Callback &);

            virtual void fam_scatter(Hermes::Vaddr src, Shmem::Fam_Descriptor rd,
                                     uint64_t nElements,
                                     uint64_t firstElement, uint64_t stride, uint64_t elementSize,
                                     bool blocking, Shmem::Callback &);

            virtual void fam_scatterv(Hermes::Vaddr src, Shmem::Fam_Descriptor fd,
                                      uint64_t nElements,
                                      std::vector <uint64_t> elementIndex, uint64_t elementSize,
                                      bool blocking, Shmem::Callback &);

            virtual void fam_gather(Hermes::Vaddr dest, Shmem::Fam_Descriptor fd,
                                    uint64_t nElements,
                                    uint64_t firstElement, uint64_t stride, uint64_t elmentSize,
                                    bool blocking, Shmem::Callback &);

            virtual void fam_gatherv(Hermes::Vaddr dest, Shmem::Fam_Descriptor fd,
                                     uint64_t nElements,
                                     std::vector <uint64_t> elementIndex, uint64_t elmentSize,
                                     bool blocking, Shmem::Callback &);

            void memcpy(Hermes::Vaddr dest, Hermes::Vaddr src, size_t length,
                        Shmem::Callback callback);
        ...
        private:
            ...
            virtual void get(Hermes::Vaddr dest, Hermes::Vaddr src, size_t nelems, int pe,
                             bool blocking, Shmem::Callback, Shmem::Callback &fini);

            virtual void put(Hermes::Vaddr dest, Hermes::Vaddr src, size_t nelems, int pe,
                             bool blocking, Shmem::Callback, Shmem::Callback &fini);

            virtual void fam_put2(Shmem::Fam_Descriptor fd, uint64_t offset, Hermes::Vaddr src,
                                  uint64_t nbytes, Shmem::Callback, Shmem::Callback);

            virtual void fam_get2(Hermes::Vaddr dest, Shmem::Fam_Descriptor fd, uint64_t offset,
                                  uint64_t nbytes, Shmem::Callback, Shmem::Callback);
            ...
            void fam_gather(FamVectorWork *work, Hermes::Vaddr dest, Shmem::Fam_Descriptor fd,
                            uint64_t nElements,
                            uint64_t elementSize, bool blocking, Shmem::Callback &callback);

            void fam_scatter(FamVectorWork *work, Hermes::Vaddr src, Shmem::Fam_Descriptor fd,
                             uint64_t nElements,
                             uint64_t elementSize, bool blocking, Shmem::Callback &callback);
            ...
            void createWorkList(uint64_t addr, uint64_t nbytes,
                                std::queue <std::pair<uint64_t, uint64_t>> &list) {
                if (!m_famAddrMapper) {
                    dbg().fatal(CALL_INFO, -1, "FAM mapping module not configured\n");
                }
                uint32_t blockSize = m_famAddrMapper->blockSize();
                dbg().debug(CALL_INFO, 2, SHMEM_BASE, "addr=%#"
                PRIx64
                " nbytes=%"
                PRIu64
                "\n", addr, nbytes);
                ...
                while (nbytes) {
                    ...
                    dbg().debug(CALL_INFO, 3, SHMEM_BASE, "seg_addr=%#"
                    PRIx64
                    " seg_nbytes=%"
                    PRIu64
                    "\n", addr, seg_nbytes);
