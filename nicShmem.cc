...
void Nic::Shmem::handleEvent(NicShmemCmdEvent *event, int id) {
    m_dbg.verbosePrefix(prefix(), CALL_INFO, 1, NIC_DBG_SHMEM,
                        "core=%d `%s` targetNode=%d targetCore=%d\n",
...
void Nic::Shmem::handleHostEvent(NicShmemCmdEvent *event, int id) {
    if (m_hostBusy) {
        ...
        m_nic.schedCallback(
            [=]() {
                m_dbg.verbosePrefix(prefix(), CALL_INFO_LAMBDA, "handleHostEvent", 1, NIC_DBG_SHMEM,
                                    "ready core=%d\n", id);
                m_hostBusy = false;
                if (!m_hostCmdQ.empty()) {
...
void Nic::Shmem::handleNicEvent(NicShmemCmdEvent *event, int id) {
    ...
    vec->push_back(MemOp(-1, 16, MemOp::Op::HostBusWrite,
                         [=]() {
                             m_dbg.verbosePrefix(prefix(), CALL_INFO_LAMBDA, "handleNicEvent", 1,
                                                 NIC_DBG_SHMEM, "latency=%"
                             PRIu64
                             "\n",
...
void Nic::Shmem::handleNicEvent2(NicShmemCmdEvent *event, int id) {
    if (m_engineBusy) {
        ...
        m_nic.schedCallback(
            ...
            [=]() {
                m_dbg.verbosePrefix(prefix(), CALL_INFO_LAMBDA, "handleNicEvent2", 1, NIC_DBG_SHMEM,
                                    "ready core=%d\n", id);
                m_engineBusy = false;
                if (!m_cmdQ.empty()) {
...
void Nic::Shmem::init(NicShmemInitCmdEvent *event, int id) {
    m_dbg.verbosePrefix(prefix(), CALL_INFO, 1, NIC_DBG_SHMEM, "core=%d simVAddr=%"
    PRIx64
    "\n",
        id, event->addr );
...
void Nic::Shmem::put(NicShmemPutCmdEvent *event, int id) {
    m_dbg.verbosePrefix(prefix(), CALL_INFO, 1, NIC_DBG_SHMEM, "core=%d targetNode=%d farAddr=%"
    PRIx64
    " len=%lu\n",
    ...
    ShmemPutSendEntry *entry = new ShmemPutbSendEntry(id, m_nic.getSendStreamNum(id), event,
                                                      getBacking(id, event->getMyAddr(),
                                                                 event->getLength()),
                                                      [=]() {
                                                          m_dbg.verbosePrefix(prefix(),
                                                                              CALL_INFO_LAMBDA,
                                                                              "put", 1,
                                                                              NIC_DBG_SHMEM,
                                                                              "finished\n");
...
void Nic::Shmem::putv(NicShmemPutvCmdEvent *event, int id) {
    m_dbg.verbosePrefix(prefix(), CALL_INFO, 1, NIC_DBG_SHMEM, "core=%d targetCore=%d far=%"
    PRIx64
    " len=%lu\n",
    ...
    ShmemPutSendEntry *entry = new ShmemPutvSendEntry(id, m_nic.getSendStreamNum(id), event,
                                                      [=]() {
                                                          m_nic.getVirtNic(id)->notifyShmem(
                                                              getNic2HostDelay_ns());
                                                          decActivePuts(id);
                                                      }
    );
...
void Nic::Shmem::getv(NicShmemGetvCmdEvent *event, int id) {
    m_dbg.verbosePrefix(prefix(), CALL_INFO, 1, NIC_DBG_SHMEM, "core=%d far=%"
    PRIx64
...
void Nic::Shmem::add(NicShmemAddCmdEvent *event, int id) {
    m_dbg.verbosePrefix(prefix(), CALL_INFO, 1, NIC_DBG_SHMEM,
                        "core=%d targetNode=%d targetCore=%d far=%"
    PRIx64
    " len=%lu event=%p\n", id,
...
void Nic::Shmem::hostWait(NicShmemOpCmdEvent *event, int id) {
    std::stringstream tmp;
    tmp << event->value;
    m_dbg.verbosePrefix(prefix(), CALL_INFO, 1, NIC_DBG_SHMEM, "core=%d addr=%"
    PRIx64
    " value=%s %s\n", id, event->addr,
    ...
    Op *op = new WaitOp(event, getBacking(id, event->addr, event->value.getLength()),
                        [=]() {
                            m_dbg.verbosePrefix(prefix(), CALL_INFO_LAMBDA, "hostWait", 1,
                                                NIC_DBG_SHMEM, "core=%d addr=%"
                            PRIx64
                            " finished\n", id, addr);
...
void Nic::Shmem::hostPut(NicShmemPutCmdEvent *event, int id) {
    ...
    if (event->getOp() == Hermes::Shmem::ReduOp::MOVE) {
        ...
        unsigned char *dest = (unsigned char *) getBacking(event->getVnic(), event->getFarAddr(),
                                                           event->getLength());
        unsigned char *src = (unsigned char *) getBacking(id, event->getMyAddr(),
                                                          event->getLength());
        ...
        vec->push_back(MemOp(event->getFarAddr(), event->getMyAddr(), event->getLength(),
                             MemOp::Op::HostCopy));

    } else {
        doReduction(event->getOp(), event->getVnic(), event->getFarAddr(), id, event->getMyAddr(),
                    event->getLength(), event->getDataType(), *vec);
    }
    ...
    m_nic.calcHostMemDelay(id, vec,
                           [=]() {
                               m_dbg.verbosePrefix(prefix(), CALL_INFO_LAMBDA, "hostPut", 1,
                                                   NIC_DBG_SHMEM, "core=%d finished latency=%"
                               PRIu64
                               "\n", id,
                                   m_nic.getCurrentSimTimeNano() - start);
...
void Nic::Shmem::hostGetv(NicShmemGetvCmdEvent *event, int id) {
    ...
    m_nic.calcHostMemDelay(id, vec,
                           [=]() {
                               ...
                               m_dbg.verbosePrefix(prefix(), CALL_INFO_LAMBDA, "hostGetv", 1,
                                                   NIC_DBG_SHMEM, "core=%d finished\n", id);
...
void Nic::Shmem::hostGet(NicShmemGetCmdEvent *event, int id) {
    ...
    vec->push_back(
        MemOp(event->getFarAddr(), event->getMyAddr(), event->getLength(), MemOp::Op::HostCopy));

    m_nic.calcHostMemDelay(id, vec,
                           [=]() {
                               m_dbg.verbosePrefix(prefix(), CALL_INFO_LAMBDA, "hostGet", 1,
                                                   NIC_DBG_SHMEM, "core=%d finished\n", id);
                               m_nic.getVirtNic(id)->notifyShmem(0, event->getCallback());
                               delete event;
                           }
...
void Nic::Shmem::hostFadd(NicShmemFaddCmdEvent *event, int id) {
    ...
    m_nic.calcHostMemDelay(id, vec,
                           [=]() {
                               ...
                               m_dbg.verbosePrefix(prefix(), CALL_INFO_LAMBDA, "hostFadd", 1,
                                                   NIC_DBG_SHMEM, "core=%d finished\n", id);
...
void Nic::Shmem::sameNodeCswap(NicShmemCswapCmdEvent *event, int id) {
    ...
    m_nic.calcNicMemDelay(m_nic.allocNicRecvUnit(id), id, vec,
                          [=]() {
                              Hermes::Value _save = save;
                              m_dbg.verbosePrefix(prefix(), CALL_INFO_LAMBDA, "sameNodeCswap", 1,
                                                  NIC_DBG_SHMEM, "core=%d finished\n", id);
                              m_nic.getVirtNic(id)->notifyShmem(0, event->getCallback(), _save);
                              delete event;
                          }
...
void Nic::Shmem::sameNodeSwap(NicShmemSwapCmdEvent *event, int id) {
    ...
    m_nic.calcNicMemDelay(m_nic.allocNicRecvUnit(id), id, vec,
                          [=]() {
                              Hermes::Value _save = save;
                              m_dbg.verbosePrefix(prefix(), CALL_INFO_LAMBDA, "sameNodeSwap", 1,
                                                  NIC_DBG_SHMEM, "core=%d finished\n", id);
                              m_nic.getVirtNic(id)->notifyShmem(0, event->getCallback(), _save);
                              delete event;
                          }
...
void Nic::Shmem::doReduction(Hermes::Shmem::ReduOp op, int destCore, Hermes::Vaddr destAddr,
                             int srcCore, Hermes::Vaddr srcAddr, size_t length,
                             Hermes::Value::Type type, std::vector <MemOp> &vec) {
...
void Nic::Shmem::checkWaitOps(int core, Hermes::Vaddr addr, size_t length) {
    m_dbg.verbosePrefix(prefix(), CALL_INFO, 3, NIC_DBG_SHMEM, "core=%d addr=%"
    PRIx64
    " len=%lu\n", core, addr, length );
