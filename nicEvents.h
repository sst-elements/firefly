...
public:
    ...
    enum Type {
        Init, RegMem, Fence, Put, Putv, Get, Getv, Wait, Add, Fadd, Swap, Cswap
    } type;
...
public:
    ...
    NicShmemPutCmdEvent(int vnic, int node, Hermes::Vaddr dest, Hermes::Vaddr src,
                        ...
                        Hermes::Shmem::ReduOp op = Hermes::Shmem::MOVE) :
        NicShmemSendCmdEvent(Put, vnic, node), destAddr(dest), srcAddr(src),
        length(length), callback(callback), op(op) {}

    NicShmemPutCmdEvent(int vnic, int node, Hermes::Vaddr dest, Hermes::Vaddr src,
                        size_t length, Hermes::Shmem::ReduOp op, Hermes::Value::Type dataType,
                        Callback callback) :
        NicShmemSendCmdEvent(Put, vnic, node), destAddr(dest), srcAddr(src),
...
public:
    ...
    NicShmemGetCmdEvent(int vnic, int node, Hermes::Vaddr dest, Hermes::Vaddr src, size_t length,
                        Callback callback) :
...
public:
    ...
    NicShmemGetvCmdEvent(int vnic, int node, Hermes::Vaddr addr, Hermes::Value::Type type,
                         Callback callback) :
...
public:
    ...
    NicShmemFaddCmdEvent(int vnic, int node, Hermes::Vaddr addr, Hermes::Value &value,
                         Callback callback) :
...
public:
    ...
    NicShmemSwapCmdEvent(int vnic, int node, Hermes::Vaddr addr, Hermes::Value &value,
                         Callback callback) :
...
public:
    ...
    NicShmemCswapCmdEvent(int vnic, int node, Hermes::Vaddr addr, Hermes::Value &cond,
                          Hermes::Value &value, Callback callback) :
        NicShmemSendCmdEvent(Cswap, vnic, node), srcAddr(addr), cond(cond), data(value),
        callback(callback) {}
...
public:
    ...
    NicShmemOpCmdEvent(Hermes::Vaddr addr, Hermes::Shmem::WaitOp op, Hermes::Value &value,
                       Callback callback) :
...
public:
    enum Type {
        PioSend, DmaSend, DmaRecv, Put, Get, RegMemRgn
    } type;
    int node;
    int dst_vNic;
    int tag;
...
public:
    enum Type {
        PioSend, DmaSend, DmaRecv, Put, Get, NeedRecv
    } type;
    int src_vNic;
    int node;
    int tag;
