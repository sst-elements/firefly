...
Nic::Nic(ComponentId_t id, Params &params) :
    ...
    m_nic2host_lat_ns = calcDelay_ns(
        params.find<SST::UnitAlgebra>("nic2host_lat", SST::UnitAlgebra("150ns")));
    ...
    m_linkRecvWidget = new LinkControlWidget(m_dbg,
                                             [=]() {
                                                 m_dbg.debug(CALL_INFO, 2, 1,
                                                             "call setNotifyOnReceive\n");
                                                 m_linkControl->setNotifyOnReceive(
                                                     m_recvNotifyFunctor);
                                             }
    );
    ...
    m_linkSendWidget = new LinkControlWidget(m_dbg,
                                             [=]() {
                                                 m_dbg.debug(CALL_INFO, 2, 1,
                                                             "call setNotifyOnSend\n");
                                                 m_linkControl->setNotifyOnSend(
                                                     m_sendNotifyFunctor);
                                             }
    );
    ...
    m_shmem = new Shmem(*this, shmemParams, m_myNodeId, m_num_vNics, m_dbg, numShmemCmdSlots,
                        getDelay_ns(), getDelay_ns());
    ...
    if (params.find<int>("useSimpleMemoryModel", 0)) {
        ...
        m_memoryModel = dynamic_cast<MemoryModel *>(loadSubComponent("firefly.SimpleMemory", this,
                                                                     smmParams));
    }
    if (params.find<int>("useTrivialMemoryModel", 0)) {
        if (m_memoryModel) {
            m_dbg.fatal(CALL_INFO, 0,
                        "can't used TrivialMemoryModel, memoryModel already configured\n");
        }
        Params smmParams = params.find_prefix_params("simpleMemoryModel.");
    ...
    for (int i = 0; i < numSendMachines - 1; i++) {
        SendMachine *sm = new SendMachine(*this, m_myNodeId,
                                          params.find<uint32_t>("verboseLevel", 0),
                                          params.find<uint32_t>("verboseMask", -1),
                                          i, packetSizeInBytes, packetOverhead, maxSendMachineQsize,
                                          allocNicSendUnit(), false);
    ...
    m_sendMachineV[numSendMachines - 1] = new SendMachine(*this, m_myNodeId,
                                                          params.find<uint32_t>("verboseLevel", 0),
                                                          params.find<uint32_t>("verboseMask", -1),
                                                          m_sendMachineV.size() - 1,
                                                          packetSizeInBytes, packetOverhead,
                                                          maxSendMachineQsize, allocNicSendUnit(),
                                                          true);
...
void Nic::dmaRecv(NicCmdEvent *e, int vNicNum) {
    DmaRecvEntry::Callback callback = std::bind(&Nic::notifyRecvDmaDone, this, vNicNum, _1, _2, _3,
                                                _4, _5);
...
void Nic::get(NicCmdEvent *e, int vNicNum) {
    ...
    m_dbg.debug(CALL_INFO, 1, 1,
                "src_vNic=%d dest=%#x dst_vNic=%d tag=%#x vecLen=%lu totalBytes=%lu\n",
                vNicNum, e->node, e->dst_vNic, e->tag, e->iovec.size(), entry->totalBytes());

    qSendEntry(
        new GetOrgnEntry(vNicNum, getSendStreamNum(vNicNum), e->node, e->dst_vNic, e->tag, getKey));
}
...
void Nic::qSendEntry(SendEntryBase *entry) {
    ...
    if (entry->totalBytes() >= MaxPayload) {
        m_dbg.fatal(CALL_INFO, -1,
                    "Error: maximum network message size must be less than %d bytes, requested size %zu\n",
...
void Nic::feedTheNetwork() {
    ...
    while (!m_sendPQ.empty()) {
        ...
        if (!ret) {
            ...
            SimTime_t curTime = Simulation::getSimulation()->getCurrentSimCycle();
            SimTime_t latPS =
                ((double) x.pkt->payloadSize() / (double) m_linkBytesPerSec) * 1000000000000;
            ...
            m_dbg.debug(CALL_INFO, 1, NIC_DBG_SEND_NETWORK, "predNetIdleTime=%lld\n",
                        m_predNetIdleTime);
            m_dbg.debug(CALL_INFO, 1, NIC_DBG_SEND_NETWORK, "p1=%"
            PRIu64
            " p2=%d\n", entry->p1(), entry->p2());
...
void Nic::detailedMemOp(Thornhill::DetailedCompute *detailed,
    ...
    for (unsigned i = 0; i < vec.size(); i++) {
        ...
        if (0 == vec[i].addr) {
            m_dbg.fatal(CALL_INFO, -1, "Invalid addr %"
            PRIx64
            "\n", vec[i].addr);
        }
        if (vec[i].addr < 0x100) {
            m_dbg.output(CALL_INFO, "Warn addr %"
            PRIx64
            " ignored\n", vec[i].addr);
            i++;
            continue;
...
Hermes::MemAddr Nic::findShmem(int core, Hermes::Vaddr addr, size_t length) {
    ...
    m_dbg.debug(CALL_INFO, 1, NIC_DBG_SHMEM, "found region core=%d Vaddr=%#"
    PRIx64
    " length=%lu\n",
    ...
    if (addr + length > region.first.getSimVAddr() + region.second) {
        m_dbg.fatal(CALL_INFO, -1, "address %#"
        PRIx64
        " not in region %#"
        PRIx64
