...
API::API(Component *owner, Params &params) :
    ...
    m_processQueuesState = dynamic_cast<ProcessQueuesState *>( owner->loadSubComponent(
        "firefly.ctrlMsg", owner, params));
...
void API::setVars(Info *info, VirtNic *nic, Thornhill::MemoryHeapLink *mem, Link *retLink) {
    ...
    char buffer[100];
    snprintf(buffer, 100, "@t:%d:%d:CtrlMsg::API::@p():@l ", nic->getRealNodeId(),
             m_info->worldRank());
