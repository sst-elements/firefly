...
void ProcessQueuesState::finish() {
    dbg().debug(CALL_INFO, 1, 1,
                "pstdRcvQ=%lu recvdMsgQ=%lu loopResp=%lu funcStack=%lu sent=%d recv=%zu\n",
                m_pstdRcvQ.size(), m_recvdMsgQ.size(), m_loopResp.size(), m_funcStack.size(),
                m_numSent, m_numRecv + m_recvdMsgQ.size());
}
...
void ProcessQueuesState::enterInit_1(uint64_t addr, size_t length) {
    dbg().debug(CALL_INFO, 1, 1, "simVAddr %"
    PRIx64
    "  length=%zu\n", addr, length );
...
void ProcessQueuesState::enterSend(_CommReq *req, uint64_t exitDelay) {
    ...
    dbg().debug(CALL_INFO, 1, DBG_MSK_PQS_APP_SIDE, "req=%p delay=%"
    PRIu64
    " destRank=%d\n", req, exitDelay,
        req->getDestRank());
...
void ProcessQueuesState::enterMakeProgress(uint64_t exitDelay) {
    ...
    WaitCtx *ctx = new WaitCtx(NULL,
                               std::bind(&ProcessQueuesState::processMakeProgress, this,
                                         &m_funcStack)
    );
...
void ProcessQueuesState::dmaRecvFiniSRB(ShortRecvBuffer *buf, nid_t nid,
    ...
    dbg().debug(CALL_INFO, 2, DBG_MSK_PQS_CB, "ShortMsgQ nid=%#x tag=%#x length=%lu\n",
                nid, tag, length);
    dbg().debug(CALL_INFO, 2, DBG_MSK_PQS_CB, "ShortMsgQ rank=%d tag=%#"
    PRIx64
    " count=%d "
...
bool ProcessQueuesState::checkMatchHdr(MatchHdr &hdr, MatchHdr &wantHdr,
                                       uint64_t ignore) {
    dbg().debug(CALL_INFO, 2, DBG_MSK_PQS_Q, "posted tag %#"
    PRIx64
    ", msg tag %#"
    PRIx64
    "\n", wantHdr.tag, hdr.tag );
