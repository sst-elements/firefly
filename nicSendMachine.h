...
class SendMachine {
    ...
    class OutQ {
        ...
        struct Entry {
            Entry(std::pair<FireflyNetworkEvent *, int> data, Callback callback = NULL) : data(
    ...
    public:
        ...
        OutQ(Nic &nic, Output &output, int myId, int maxQsize) :
            ...
            m_prefix = "@t:" + std::to_string(nic.getNodeId()) + ":Nic::SendMachine" +
                       std::to_string(myId) + "::OutQ::@p():@l ";
        }
    ...
    public:
        ...
        InQ(Nic &nic, Output &output, int myId, OutQ *outQ, int maxQsize) :
            m_nic(nic), m_dbg(output), m_outQ(outQ), m_numPending(0), m_maxQsize(maxQsize),
            m_callback(NULL),
            m_pktNum(0), m_expectedPkt(0) {
            m_prefix = "@t:" + std::to_string(nic.getNodeId()) + ":Nic::SendMachine" +
                       std::to_string(myId) + "::InQ::@p():@l ";
        }
        ...
        void enque(int unit, int pid, std::vector <MemOp> *vec, FireflyNetworkEvent *ev, int dest,
