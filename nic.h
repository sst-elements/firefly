...
#define STREAM_NUM_SIZE 12
        ...
        private:
            ...
            public:
                LinkControlWidget(Output &output, Callback callback) : m_dbg(output),
                                                                       m_notifiers(1, NULL),
                                                                       m_num(0),
                                                                       m_callback(callback) {
                }
                ...
                inline bool notify(int vn) {
                    ...
                    if (m_notifiers[vn]) {
                        m_dbg.debug(CALL_INFO, 1, NIC_DBG_LINK_CTRL,
                                    "Widget call notifier, number still installed %d\n", m_num - 1);
                        m_notifiers[vn]();
                        m_notifiers[vn] = NULL;
                        --m_num;
                    }
                ...
                inline void setNotify(std::function<void()> notifier, int vn) {
                    m_dbg.debug(CALL_INFO, 1, NIC_DBG_LINK_CTRL,
                                "Widget vn=%d, number now installed %d\n", vn, m_num + 1);
                    if (m_num == 0) {
                        m_callback();
                    }
        ...
        private:
            ...
            struct __attribute__ ((packed)) ShmemMsgHdr {
                ...
                uint32_t length;
                enum Op {
                    Ack, Put, Get, GetResp, Add, Fadd, Swap, Cswap
                };
                unsigned char op : 3;
                unsigned char op2 : 3;
...
#include "nicUnitPool.h"
        ...
        private:
            ...
            struct X {
                X(Callback callback, FireflyNetworkEvent *pkt, int dest) : callback(callback),
                                                                           pkt(pkt), dest(dest) {}
            ...
            std::priority_queue<PriorityX *, std::vector < PriorityX * >, Compare >
            m_sendPQ;
            ...
            void notifyHavePkt(PriorityX *px) {
                m_dbg.debug(CALL_INFO, 3, NIC_DBG_SEND_MACHINE, "p1=%"
                PRIu64
                " p2=%d\n", px->p1(), px->p2());
                m_sendPQ.push(px);
            ...
            void calcHostMemDelay(int core, std::vector <MemOp> *ops,
            ...
            void calcNicMemDelay(int unit, int pid, std::vector <MemOp> *ops,
                                 std::function<void()> callback) {
                if (m_memoryModel) {
