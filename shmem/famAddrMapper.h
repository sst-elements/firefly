    ...
    namespace Firefly {
        ...
        public:
            ...
            void setDbg(Output *output) {
                m_dbg = output;
                m_dbg->debug(CALL_INFO, 3, 0, "numNodes=%d bytesPerNode=%"
                PRIu64
                " blockSize=%"
                PRIu64
                "\n",
        ...
        public:
            RR_FamAddrMapper(Params &params) {
                m_bytesPerNode = (size_t) params.find<SST::UnitAlgebra>("bytesPerNode",
                                                                        "16MiB").getRoundedValue();
                m_blockSize = (size_t) params.find<SST::UnitAlgebra>("blockSize",
                                                                     "4KiB").getRoundedValue();
            ...
            virtual void getAddr(uint64_t globalOffset, int &node, uint64_t &localOffset) {
                ...
                if (block >= m_totalBlocks) {
                    m_dbg->fatal(CALL_INFO, -1, "RR_FamAddrMapper addr 0x%"
                    PRIx64
                    " is out of range maxAddr 0x%"
                    PRIx64
                    "\n",
                ...
                m_dbg->debug(CALL_INFO, 3, 0, "globalOffset=%#"
                PRIx64
                " blockNum=%"
                PRIu64
                " node=%#x localOffset=0x%"
                PRIx64
                "\n",
