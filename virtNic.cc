...
void VirtNic::shmemGet(int node, Hermes::Vaddr dest, Hermes::Vaddr src, size_t len,
                       Callback callback) {
    m_dbg.debug(CALL_INFO, 2, 0, "\n");
    sendCmd(0, new NicShmemGetCmdEvent(calcCoreId(node), calcRealNicId(node), dest, src, len,
                                       callback));
}
...
void VirtNic::shmemGetv(int node, Hermes::Vaddr src, Hermes::Value::Type type, CallbackV callback) {
    m_dbg.debug(CALL_INFO, 2, 0, "\n");
    sendCmd(0,
            new NicShmemGetvCmdEvent(calcCoreId(node), calcRealNicId(node), src, type, callback));
}

void VirtNic::shmemWait(Hermes::Vaddr addr, Hermes::Shmem::WaitOp op, Hermes::Value &value,
                        Callback callback) {
    m_dbg.debug(CALL_INFO, 2, 0, "\n");
    sendCmd(0, new NicShmemOpCmdEvent(addr, op, value, callback));
}

void VirtNic::shmemPut(int node, Hermes::Vaddr dest, Hermes::Vaddr src, size_t len,
                       Callback callback) {
    m_dbg.debug(CALL_INFO, 2, 0, "\n");
    sendCmd(0, new NicShmemPutCmdEvent(calcCoreId(node), calcRealNicId(node), dest, src, len,
                                       callback));
}
...
void VirtNic::shmemPutOp(int node, Hermes::Vaddr dest, Hermes::Vaddr src, size_t len,
    ...
    sendCmd(0, new NicShmemPutCmdEvent(calcCoreId(node), calcRealNicId(node), dest, src, len, op,
                                       dataType, callback));
}
...
void VirtNic::shmemSwap(int node, Hermes::Vaddr dest, Hermes::Value &value, CallbackV callback) {
    m_dbg.debug(CALL_INFO, 2, 0, "\n");
    sendCmd(0,
            new NicShmemSwapCmdEvent(calcCoreId(node), calcRealNicId(node), dest, value, callback));
}

void VirtNic::shmemCswap(int node, Hermes::Vaddr dest, Hermes::Value &cond, Hermes::Value &value,
                         CallbackV callback) {
    m_dbg.debug(CALL_INFO, 2, 0, "\n");
    sendCmd(0, new NicShmemCswapCmdEvent(calcCoreId(node), calcRealNicId(node), dest, cond, value,
                                         callback));
}
...
void VirtNic::shmemFadd(int node, Hermes::Vaddr dest, Hermes::Value &value, CallbackV callback) {
    m_dbg.debug(CALL_INFO, 2, 0, "\n");
    sendCmd(0,
            new NicShmemFaddCmdEvent(calcCoreId(node), calcRealNicId(node), dest, value, callback));
}
