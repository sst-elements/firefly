...
class Shmem {
    ...
    public:
        ...
        Op(Type type, NicShmemOpCmdEvent *cmd, Callback callback) : m_type(type), m_cmd(cmd),
                                                                    m_callback(callback) {}
    ...
    public:
        ...
        bool checkOp(Output &dbg, int core) {
            ...
            tmp << "op=" << WaitOpName(m_cmd->op) << " testValue=" << m_cmd->value << " memValue="
                << m_value;
...
public:
    Shmem(Nic &nic, Params &params, int id, int numVnics, Output &output, int numCmdSlots,
          SimTime_t nic2HostDelay_ns, SimTime_t host2NicDelay_ns) :
        m_nic(nic), m_dbg(output), m_one((long) 1),
        m_nic2HostDelay_ns(nic2HostDelay_ns), m_host2NicDelay_ns(host2NicDelay_ns),
        m_engineBusy(false), m_hostBusy(false) {
    ...
    std::pair <Hermes::MemAddr, size_t> &findRegion(int core, uint64_t addr) {
        ...
        m_dbg.fatal(CALL_INFO, 0, " core %d Unable to find for for addr %"
        PRIx64
        "\n", core, addr);
...
private:
    ...
    std::vector <std::vector<std::pair < Hermes::MemAddr, size_t>> >
    m_regMem;
